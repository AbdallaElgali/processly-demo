  ...
  return (
    <Box sx={{ height: '100%', width: '100%', bgcolor: colors.surface, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      <Box sx={{ p: 1, bgcolor: colors.surface, borderBottom: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
        <Tooltip title="Zoom Out"><IconButton onClick={handleZoomOut} size="small"><RemoveIcon /></IconButton></Tooltip>
        <Typography variant="body2" sx={{ minWidth: '45px', textAlign: 'center', fontWeight: 'bold' }}>{Math.round(scale * 100)}%</Typography>
        <Tooltip title="Zoom In"><IconButton onClick={handleZoomIn} size="small"><AddIcon /></IconButton></Tooltip>
        <Divider orientation="vertical" flexItem sx={{ mx: 1 }} />
        <Tooltip title="Reset"><IconButton onClick={handleResetZoom} size="small"><RestartAltIcon /></IconButton></Tooltip>
        <Tooltip title="Fit Width"><IconButton onClick={handleFitWidth} size="small"><ZoomInMapIcon /></IconButton></Tooltip>
      </Box>

      <Box ref={containerRef} sx={{ flex: 1, overflow: 'auto', p: 2, display: 'flex', flexDirection: 'column', gap: 3, scrollbarWidth: 'thin' }}>
        {pdfDocument ? (
          <Document file={pdfDocument} onLoadSuccess={({ numPages }) => setNumPages(numPages)} loading={<CircularProgress sx={{ mt: 10, color: 'white' }} />}>
            {Array.from(new Array(numPages), (_, index) => {
              ...
              return (
                <Box key={pageNum} ref={(el: HTMLDivElement | null) => { pageRefs.current[pageNum] = el; }} sx={{ position: 'relative', boxShadow: '0px 4px 20px rgba(0,0,0,0.3)', transition: 'width 0.2s ease-out', mx: 'auto' }}>
                  <Page 
                    pageNumber={pageNum} 
                  ...
                  {isHighlighted && highlightStyle && (
                    <Box sx={{ position: 'absolute', border: `2px solid ${colors.primary}`, backgroundColor: 'rgba(25, 118, 210, 0.2)', pointerEvents: 'none', zIndex: 10, boxShadow: `0 0 0 9999px rgba(0, 0, 0, 0.1)`, transition: 'all 0.3s ease', ...highlightStyle }} />
                  )}
                </Box>
              );
            })}
          ...
          <Box sx={{ mt: 20, textAlign: 'center', color: 'rgba(255,255,255,0.5)', mx: 'auto' }}><Typography>Upload a document to view</Typography></Box>
        )}
      </Box>

      {activeHighlight?.textSnippet && (
        <Paper elevation={6} sx={{ p: 2, borderTop: `1px solid ${colors.border}`, bgcolor: colors.surface, zIndex: 20, display: 'flex', flexDirection: 'column', gap: 0.5 }}>
